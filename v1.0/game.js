(() => {
  ...
  function updateStatus() {
    ...
    document.querySelector(".score-box.p1").classList.toggle("out", balls && balls[0].out);
    document.querySelector(".score-box.p2").classList.toggle("out", balls && balls[1].out);
  }
  ...
  function showOverlay(title, cls, withRestart) {
    ...
    if (mode === "menu") {
      ...
      const bai = mkBtn("vs AI", "btn secondary", () => startGame(true));
  ...
  // ---- canvas sizing: keep internal resolution crisp on hi-dpi ----
  function resize() {
    ...
    // recenter pivot in CSS px space (logical 720x480)
    PIVOT.x = 360;
    PIVOT.y = 260;
